# Written by Lauri Tuppi

# This is an example script that calculates value of the cost function required for 
# parameter estimation using the EPPES or DE algorithm. In this script scaled moist total energy 
# norm of whole atmosphere is used as metrics of forecast performance. The moist total 
# energy norm calculation algorithm has been coded in two ways according to equation [2.2] in Ehrendorfer et al. (1999), and equation [2] in 
...
# into a file for later use. This script requires both forecast and reference U wind, V wind, temperature, specific humidity,
...
# 7. Selection between two alternative ways to calculate the energy norm. 1 = Ehrendorfer's style, and 2 = Ollinaho's style
...
# References:
# Ehrendorfer, M. et al., 1999. Singular-Vector Perturbation Growth in a Primitive Equation Model with Moist Physics.
#	Journal of the Atmospheric Sciences, 56, pp.1627-1648.
...
split = int(sys.argv[8])
scale = float(sys.argv[9])	 # an ad hoc scaling term of the cost function

print('########################### input arguments')
print('forecastUVT=', forecastUVT)
print('forecastP=', forecastP)
print('ref_step=', ref_step)
print('referenceUVT=', referenceUVT)
print('referenceP=', referenceP)
print('outputdir=', outputdir)
print('calc_style=', calc_style)
print('split=', split)
print('scale=', scale)
print('###########################')
...
if split==0:
  ...
  print '+', dlnp2, ' logarithm of surface pressure'
  print('--------------------------')
  deltaE2 = du2 + dv2 + dt2 + dlnp2
  print ' ', deltaE2, ' <- dry total energy norm', dq2, '<- moist term'
  print('!!! Split cost function: dry norm and moist term as seperate cost functions!!!')
  dten[0] = (deltaE2 * dq2) ** 0.5
  print 'Costfunction after geometric mean: ', dten
  np.savetxt(outputdir + '/score.dat', dten)
