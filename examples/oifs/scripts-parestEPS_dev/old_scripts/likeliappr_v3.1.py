...
tcol_ens = np.zeros((nlevs*ncol,nensm))
tcol_c = np.zeros(nlevs*ncol)

for i in range(ncol):					# append the ncol random columns into long colums
  start = i*nlevs
  stop = (i+1)*nlevs
  lon = sel_lon[i]
  lat = sel_lat[i]
  tcol_c[start:stop] = t_c[::-1,lat,lon]		# reshaping the control data into one column
  ...
  for j in range(0,nensm):				# ens data is reshaped into nensmm columns similar way as the control data
    tcol_ens[start:stop,j] = ens_t_data[j,:,lat,lon]

s2_obs = 0.8**2						# obsevation error variance (a rough guess)

ens_mean = (sum(tcol_ens.T))/float(nensm)

#for i in range(nlevs*ncol): # test to check that the data is read correctly
#  print tcol_c[i], ens_mean[i]

apu1 = np.zeros(nlevs*ncol)
for i in range(0,nensm):				# calculation of ensemble variance
  apu1 += (tcol_ens[:,i]-ens_mean)**2

s2_ens = sum(apu1/(float(nensm)-1.0))/float(nlevs*ncol)

Jk = 0.0
for j in range(0,nlevs*ncol):				# calculation of the cost function part of the approx likelihood function
  Jk = Jk + (tcol_c[j] - ens_mean[j])**2 / (s2_obs + s2_ens)
